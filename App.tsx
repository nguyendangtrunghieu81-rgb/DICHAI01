...
function App() {
  ...
  const minIntervalRef = useRef<number>(1000); // Throttling: minimum 1s between translations
  ...
  const handleAiError = useCallback((error: any) => {
    if (error?.status === 429 || error?.message?.includes('429')) {
      ...
      setAiError("API Quota Exceeded. Background tasks paused for 30s.");
      ...
      setAiError("AI processing hiccup. Retrying...");
      setTimeout(() => setAiError(null), 5000);
    }
  }, []);

  const refineEnglishTranscript = useCallback(async () => {
    if (!process.env.API_KEY || refiningInProgressRef.current || quotaExceeded) return;
    
    const raw = text;
    const unrefinedChunk = raw.slice(refinedRef.current).trim();
    
    // Higher threshold to reduce request count
    ...
    try {
        ...
        const sys = `Expert AI Transcriber. Polish speech to text transcript.
                     CONTEXT: ${contextDesc}. 
                     MATERIALS: ${fileContent.slice(0, 1000)}. 
                     Rules: Fix grammar/homophones. Keep exact meaning. English only.`;
  ...
  }, [text, contextDesc, fileContent, setText, quotaExceeded, handleAiError]);

  const performTranslation = useCallback(async (isBatchRequest: boolean = false) => {
    if (!process.env.API_KEY || (translatingRef.current && !isBatchRequest) || quotaExceeded) return;
    
    const sourceText = text;
    // We translate the current refined or raw chunk
    const endPoint = sourceText.length; 
    const chunk = sourceText.slice(processedRef.current, endPoint);
    
    // Minimum 15 chars for a meaningful translation request
    if (chunk.trim().length < 15 && !isBatchRequest) return;
    ...
    try {
        ...
        let sys = `Professional Simultaneous Interpreter (EN -> VI).
                   CONTEXT: ${contextDesc}.
                   TECHNICAL: ${fileContent.slice(0, 2000)}.
                   Output ONLY Vietnamese. Accurate, concise.`;

        const res = await ai.models.generateContent({
            model: activeModel,
            contents: `Translate: "${chunk}"`,
            config: { systemInstruction: sys, temperature: 0.1 }
        });
  ...
  }, [text, contextDesc, fileContent, quotaExceeded, handleAiError]);

  // Orchestration with safer intervals to respect API limits
  useEffect(() => {
    if (!isAuthenticated || isLiveMode || isBatchMode || quotaExceeded) return;

    // Background Refiner
    if (!refiningInProgressRef.current && text.length > refinedRef.current + 30) {
        if (refineTimeoutRef.current) clearTimeout(refineTimeoutRef.current);
        refineTimeoutRef.current = setTimeout(() => refineEnglishTranscript(), 3000);
    }

    // Background Translator
    if (!translatingRef.current && text.length > processedRef.current + 20) {
        if (retryTimeoutRef.current) clearTimeout(retryTimeoutRef.current);
        retryTimeoutRef.current = setTimeout(() => performTranslation(), 5000);
    }
  ...
  }, [text, refineEnglishTranscript, performTranslation, isAuthenticated, isLiveMode, isBatchMode, quotaExceeded]);
        ...
        const transcribeResponse = await ai.models.generateContent({
            ...
            contents: { 
                parts: [
                    ...
                    { text: `Transcribe this audio file. Context: ${contextDesc}` }
                ] 
            }
        });
        if (transcribeResponse.text) {
            setText(prev => prev + "\n[UPLOAD]: " + transcribeResponse.text);
        }
                ...
                <div className="flex items-center gap-4">
                    ...
                    {aiError && (
                      <div className="flex items-center gap-2 border-l border-slate-800 pl-4">
                        ...
                        <span className="text-[9px] font-black uppercase text-amber-500 animate-pulse">{aiError}</span>
                      </div>
                    )}
                </div>
            </div>
